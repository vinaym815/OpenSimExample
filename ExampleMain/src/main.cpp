...
/*
In this example a block is contrained to move on a circle by means of a constant distance
constraint and a contact with ground, and then forces are applied to it by two muscles and
an an external agent. After setting up the system forward simulation is performed.

This example teaches you how to
create physical frames and attach geometries to the
create a body with certain mass, inertia, and geometry
create a joint between two frames and adjust the joint limits
model a constant distance constraint
add muscles, and prescribe their controller
a external force
add a contact between ground and a mesh
how to intialize the model
perform a forward simulation
how to save the save the model, and stream and simulation results
*/

/*
Try to write state trajectory using statetrajectoryreporter function
*/
...
int main()
{
    ...
    try {
		...
		ground.addComponent(leftAnchorFrame);
        // Right brick
		...
		ground.addComponent(rightAnchorFrame);
		...
		// set_color is not a const function therefore we cannot use the get_Appearance
		...
		// get_color is a const function therefore we can use both get_Appearance and upd
		//	appearance to get the same results
		//bool answer = leftAnchorGeometry->get_Appearance().get_color() == leftAnchorGeometry->get_Appearance().get_color();
		//std::cout << answer << std::endl;

        // Anchor block is 0.1 by 0.1 by 0.1m cube and centered at origin.
        ...
        blockToGround->updCoordinate(FreeJoint::Coord::TranslationX).setName("xTranslation");
        blockToGround->updCoordinate(FreeJoint::Coord::TranslationZ).setName("zTranslation");
		...
		// The gravity[1] variable has a negative value
		...
		prescribedForce->setName("ExternalForceBlock");
		...
		// Use ConsoleReporterVec3 for displaying Vec3 Outputs
		ConsoleReporter* consoleReporter = new ConsoleReporter();
		consoleReporter->set_report_time_interval(0.1);
		consoleReporter->addToReport(osimModel.getComponent("/jointset/blockToGround/xTranslation").getOutput("value"), "xValue");
		consoleReporter->addToReport(osimModel.getComponent("/jointset/blockToGround/zTranslation").getOutput("value"), "zValue");
		osimModel.addComponent(consoleReporter);
		...
		// Enabling and disabling these two commands doen't make any difference
		// Drops the realization to Instance stage 
