...
public:
    ...
    //      lengthening speed).
    OpenSim_DECLARE_OUTPUT(speed, double, getSpeed, SimTK::Stage::Position);

    // The force transmitted by the device.
    //TODO: Add an output called "tension".
    OpenSim_DECLARE_OUTPUT(tension, double, getTension, SimTK::Stage::Position);

    // The power produced(+) or dissipated(-) by the device.
    //TODO: Add an output called "power".
    OpenSim_DECLARE_OUTPUT(power, double, getPower, SimTK::Stage::Position);
    ...
    OpenSim_DECLARE_OUTPUT(getCenterOfMassHeight, double, getHeight, SimTK::Stage::Position);
    ...
    double getHeight(const SimTK::State& s) const {
        const std::string hopperHeightCoord = "/jointset/slider/yCoord"; //fill this in
		return getModel().getComponent(hopperHeightCoord).getOutputValue<double>(s, "value");
    }
